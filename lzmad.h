...
#define MEM_ALLOC(size)                                        \
    ({                                                         \
        void *_ptr = mmap(0, size, PROT_READ | PROT_WRITE,     \
                          MAP_PRIVATE | MAP_ANONYMOUS, -1, 0); \
        _ptr == MAP_FAILED ? 0 : _ptr;                         \
    })

#define MEM_FREE(ptr, size) munmap(ptr, size)
...
#define K_NORMALIZE_THRESHOLD    (1 << 24)
#define K_MOVE_BITS              5
#define K_N_BIT_MODEL_TOTAL_BITS 11
#define K_BIT_MODEL_TOTAL        (1 << K_N_BIT_MODEL_TOTAL_BITS)
#define K_HALF_PROBABILITY       (K_BIT_MODEL_TOTAL >> 1)
...
{
    if (d->range < K_NORMALIZE_THRESHOLD) {
...
static uint32_t rangeDecodeBit(RangeDecoder_t *d, uint16_t *p_prob)
{
    ...
    uint32_t bound;
    rangeDecodeNormalize(d);
    bound = (d->range >> K_N_BIT_MODEL_TOTAL_BITS) * prob;
    if (d->code < bound) {
        d->range = bound;
        *p_prob = (uint16_t) (prob +
                              ((K_BIT_MODEL_TOTAL - prob) >> K_MOVE_BITS));
        return 0;
    } else {
        d->range -= bound;
        d->code -= bound;
        *p_prob = (uint16_t) (prob - (prob >> K_MOVE_BITS));
        return 1;
    }
}
                              ...
                              uint32_t match_byte)
{
    uint32_t i, val = 1, off0 = 0x100,
                off1;  // off0 and off1 can only be 0x000 or 0x100
    for (i = 0; i < 8; i++) {
        match_byte <<= 1;
        off1 = off0;
        off0 &= match_byte;
        if (!rangeDecodeBit(
                d, (p_prob + (off0 + off1 + val - 1)))) {  // get bit 0
            val <<= 1;
            off0 ^= off1;
        } else {  // get bit 1
            val <<= 1;
        ...
        case 11:
            return (type == PKT_LIT)        ? 5
                   ...
                   : (type == PKT_SHORTREP) ? 11
                                            : 11;
        default:
            return 0xFF;  // 0xFF is invalid state which will never appear
    }
}
...
#define INIT_PROBS_LITERAL(probs)                                       \
    {                                                                   \
        uint16_t *p = (uint16_t *) (probs);                             \
        uint16_t *q = p + (N_LIT_POS_STATES * N_PREV_BYTE_LC_MSBS * 3 * \
                           (1 << 8));                                   \
        for (; p < q; p++)                                              \
            *p = K_HALF_PROBABILITY;                                    \
    }  // all probabilities are init to 50% (half probability)
...
static int lzmaDecode(uint8_t *p_src, size_t src_len, uint8_t *p_dst,
                      size_t *p_dst_len, uint8_t lc, uint8_t lp, uint8_t pb)
{
    ...
    // uint16_t probs_literal  [N_LIT_POS_STATES] [N_PREV_BYTE_LC_MSBS]
    // [3*(1<<8)];

    uint16_t(*probs_literal)[N_PREV_BYTE_LC_MSBS][3 * (1 << 8)];

    size_t arr_memsiz = sizeof(uint16_t) * N_PREV_BYTE_LC_MSBS *
                        N_LIT_POS_STATES * 3 * (1 << 8);

    probs_literal = (uint16_t(*)[N_PREV_BYTE_LC_MSBS][3 * (1 << 8)])
        MEM_ALLOC(arr_memsiz);  // since this array is quiet large (3145728
                                // items, 6MB), we need to use MEM_ALLOC
    ...
    while (pos < *p_dst_len) {  // main loop
        ...
        if (!rangeDecodeBit(
                &coder,
                &probs_is_match[state][pos_state])) {  // decoded bit sequence
                                                       // = 0     (packet LIT)
            type = PKT_LIT;
        } else if (!rangeDecodeBit(
                       &coder,
                       &probs_is_rep[state])) {  // decoded bit sequence = 10
                                                 // (packet MATCH)
            type = PKT_MATCH;
        } else if (!rangeDecodeBit(
                       &coder,
                       &probs_is_rep0[state])) {  // decoded bit sequence = 110
                                                  // (packet SHORTREP or
                                                  // LONGREP0)
            type = rangeDecodeBit(&coder,
                                  &probs_is_rep0_long[state][pos_state])
                       ? PKT_REP0
                       : PKT_SHORTREP;
        } else if (!rangeDecodeBit(
                       &coder,
                       &probs_is_rep1[state])) {  // decoded bit sequence =
                                                  // 1110  (packet LONGREP1)
            type = PKT_REP1;
        } else {
            type = rangeDecodeBit(&coder, &probs_is_rep2[state]) ? PKT_REP3
                                                                 : PKT_REP2;
        }
        ...
        if (type == PKT_LIT) {
            if (state < N_LIT_STATES) {
                prev_byte = rangeDecodeInt(
                    &coder,
                    probs_literal[literal_pos_state][prev_byte_lc_msbs], 8);
            } else {
                ...
                prev_byte = rangeDecodeMB(
                    &coder,
                    probs_literal[literal_pos_state][prev_byte_lc_msbs],
                    match_byte);
            }
        }
        ...
        if (len == 0) {  // unknown length, need to decode
            ...
            if (!rangeDecodeBit(&coder, &probs_len_choice[is_rep]))
                len = 2 + rangeDecodeInt(&coder,
                                         probs_len_low[is_rep][pos_state],
                                         3);  // len = 2~9
            else if (!rangeDecodeBit(&coder, &probs_len_choice2[is_rep]))
                len = 10 + rangeDecodeInt(&coder,
                                          probs_len_mid[is_rep][pos_state],
                                          3);  // len = 10~17
            else
                len = 18 + rangeDecodeInt(&coder, probs_len_high[is_rep],
                                          8);  // len = 18~273
        }
        ...
        if (type == PKT_MATCH) {  // unknown distance, need to decode
            ...
            dist_slot = rangeDecodeInt(&coder,
                                       probs_dist_slot[len_min5_minus2],
                                       6);  // decode distance slot (0~63)
            bcnt = (dist_slot >> 1) - 1;
            ...
            if (dist_slot >= 14) {  // dist slot = 14~63
                ...
                dist |= bitsReverse(
                    rangeDecodeInt(&coder, probs_dist_align, 4), 4);
            } else if (dist_slot >= 4) {  // dist slot = 4~13
                dist |= bitsReverse(
                    rangeDecodeInt(&coder, probs_dist_special[dist_slot - 4],
                                   bcnt),
                    bcnt);
            } else {  // dist slot = 0~3
                dist = dist_slot;
    ...
    MEM_FREE(probs_literal, arr_memsiz);
...
static int parseLzmaHeader(uint8_t *p_src, uint8_t *p_lc, uint8_t *p_lp,
        ...
        *p_uncompressed_len = 0;
        for (i = 0; i < 8; i++) {
            if (i < sizeof(size_t)) {
                *p_uncompressed_len |=
                    (((size_t) p_src[5 + i])
                     << (i << 3));  // get (sizeof(size_t)) bytes from p_src,
                                    // and put it to (*p_uncompressed_len)
            } else if (p_src[5 + i] > 0) {
                return R_ERR_OUTPUT_OVERFLOW;  // uncompressed length overflow
...
static int lzma_decompress(uint8_t *p_src, size_t src_len, uint8_t *p_dst,
                           size_t *p_dst_len)
{
    ...
    RET_WHEN_ERR(parseLzmaHeader(p_src, &lc, &lp, &pb, &dict_len,
    ...
    if (uncompressed_len_known) {
        ...
        *p_dst_len = uncompressed_len;
        ...
        // printf("[LZMAd] uncompressed length is not in header, decoding using
        // output buffer length = %lu\n" , *p_dst_len);
    }

    RET_WHEN_ERR(lzmaDecode(p_src + LZMA_HEADER_LEN, src_len - LZMA_HEADER_LEN,
